...
Be thorough - find ALL possible identifiers. Err on the side of including possible matches.`;
  ...
  function requireDebtPermission(session: UserSession, permission: string, res: any): boolean {
  ...
  function injectAuditFields(session: UserSession, body: any, options?: { isReview?: boolean; isTermination?: boolean }): any {
    const now = new Date().toISOString();
