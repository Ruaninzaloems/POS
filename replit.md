...
- **Platinum Inzalo EMS API** (`georgeplatinumuatapi.azurewebsites.net`): Full POS system API authenticated via JWT bearer tokens. Auth module in `server/platinum-auth.ts` handles token management with auto-refresh. Credentials stored as environment secrets (PLATINUM_API_PASSWORD) and env vars (PLATINUM_API_USERNAME, PLATINUM_API_DBNAME). OpenAPI spec in `platinum-openapi.json`.
  ...
  - **BILLING API LIMITATION**: The `submit-consumer-payment` endpoint only accepts `paymentType=1` (Cash). Submitting with paymentType=3 (Credit Card) returns `{isSuccess: false}`. All POS payments are therefore submitted as paymentType=1 to the billing system, regardless of actual tender type. The actual tender method (cash/card) is tracked in the receipt metadata and day-end reconciliation for accounting purposes.
