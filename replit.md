...
Direct deposit manual allocations pass `userId` as the real logged-in user's ID in the `submit-details-data` payload. The `VirtualCashierUserId = -1` constant is defined for reference but is **not** sent in the payload — the API only needs `userId` (the logged-in user) and `cashFloat: 0`. All `/direct-deposits` paths are exempt from active cashier session enforcement; users can access and process direct deposit allocations without having an open cashier session. The allocation submission order is: Account/Prepaid → Group → Clearance → Direct Income → Cashbook.

The allocation search has scoped tabs: All, Account, Prepaid, Institution, Grouping, Clearance, and Income. **Institution search** uses `searchInstitutions(query)` to find institutions by name, then `fetchAccountsByGroup(institutionId)` to load all linked accounts. When an institution is selected, all linked accounts are added as `ACCOUNT` allocation lines with amounts distributed from the remaining budget (capped by each account's outstanding balance). Amounts on all allocation lines are editable inline after being added.
