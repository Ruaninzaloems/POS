...
Manual allocations submit directly to `billing-direct-deposit-allocation/submit-details-data` — **no virtual cashier session needed**. The server only injects `userId` from the authenticated session. The API resolves the cashier internally from the auth token. The server explicitly strips `cashierId`, `cashOfficeId`, `isVirtual`, `cashFloat`, and `note` from the payload before forwarding to Platinum.

**Payload specs per billType (from Kiran, 08/03/2026):**
- **BillType "1" (Consumer Services):** `{ billType, accountId, paidAmount, paymentTypeId: 5, posItemId, reconId, userId, financialYear, transactionDate, groupId, reference, description }`
- **BillType "6" (Clearance):** `{ billType, accountId, clearanceId, paidAmount, paymentTypeId: 5, posItemId, reconId, userId, financialYear, transactionDate, groupId, reference }`
- **BillType "4" (Miscellaneous):** `{ billType, amount, vatAmount, totalAmount, paidAmount, paymentTypeId: 5, posItemId, miscPaymentGroupId, reconId, userId, financialYear, transactionDate, receiptDate, groupId, lastName, initials, description, reference }`

`groupId` is sourced from `bankReconID` on the POS item. `paymentTypeId` is always `5` (EFT). `/direct-deposits` paths are exempt from active cashier session enforcement. The allocation submission order is: Account/Prepaid → Group → Clearance → Direct Income → Cashbook. Group payments use billType "1" (Consumer Services). The allocation search includes scoped tabs for various categories, with Institution search allowing discovery and allocation to linked accounts.

**Server-Side Batch Processing:** When a user clicks "Post Allocation", all lines are submitted as a single batch to the server (`POST /api/dd-allocation/submit-batch`). The server immediately returns a `jobId` and processes each line sequentially against the Platinum API in the background. The client polls `GET /api/dd-allocation/job/:jobId` every 1.5s for progress. This ensures processing completes even if the browser is closed or internet drops. Duplicate prevention: only one job per `posItemId` can be `PROCESSING` at a time. Token refresh on 401: if a line gets a 401, the server refreshes the auth token and retries once. Stale job cleanup: jobs stuck in `PROCESSING` for >15 minutes are auto-marked as `FAILED`; completed jobs are cleaned up after 1 hour. In-memory job tracking (no local DB) — appropriate for single-instance prototype.
