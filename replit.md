...
- **Direct Deposits**: Auto-loading smart grid with allocation features (auto-allocate with AI matching, manual allocation, multi-source match resolution, CSV import, bank statement history matching), allocation history, and bulk CSV processing. Allocation submission uses a resilient batch job system with exponential backoff retry (up to 4 retries per line), per-posItemId concurrency locks, and active job detection. Frontend polling is time-based (up to 5 min) with adaptive intervals and network-blip tolerance (5 consecutive failures before giving up). Allocate-once protection prevents double allocation via session-tracked `allocatedIds` and server-side locks.
