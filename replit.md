...
Direct deposit manual allocations use a **virtual cashier session** for receipt processing. The flow is: (1) create virtual cashier session (`isVirtual: true`) before the batch, (2) submit all allocation lines using the virtual cashier's ID, (3) close the virtual session after the batch completes (no full day-end reconciliation needed). The server passes `userId`, `cashierId`, and `cashOfficeId` in the `submit-details-data` payload. The server **always** resolves all three from the authenticated session and `validate-cashier` API — client-provided values are overwritten for security. If `cashierId` cannot be resolved, the server returns HTTP 400 and does not call Platinum. When a virtual session is active (`session.ddVirtualCashierId`), its ID is used instead of the real cashier ID, and `isVirtual: true` is included in the payload. The field `paymentReference` (not `reference`) is the correct API field name for generic imports. `paymentTypeId` must always be included (read from request, not defaulted by API). `/direct-deposits` paths are exempt from active cashier session enforcement. The allocation submission order is: Account/Prepaid → Group → Clearance → Direct Income → Cashbook. Group payments use billType "1" (Consumer Services). The POS uses billTypes `"1"` (Consumer Services), `"4"` (Miscellaneous), and `"6"` (Clearance). The allocation search includes scoped tabs for various categories, with Institution search allowing discovery and allocation to linked accounts.
