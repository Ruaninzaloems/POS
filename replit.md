...
8. **Session Detection**: `ActiveCashierDetails.isActive` is UNRELIABLE — returns `true` even when POS_Cashier.IsActive=0 in the database. Sessions are ONLY started via explicit `submit-cashier-setup` POST on the cashier setup page. No auto-resume. The session badge shows local session state only.
