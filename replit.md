...
Direct deposit manual allocations pass both `userId` and `cashierId` in the `submit-details-data` payload. The server resolves `cashierId` from `validate-cashier` API before forwarding to Platinum. The field `paymentReference` (not `reference`) is the correct API field name. `paymentTypeId` must always be included (read from request, not defaulted by API). `/direct-deposits` paths are exempt from active cashier session enforcement. The allocation submission order is: Account/Prepaid → Group → Clearance → Direct Income → Cashbook. Group payments use billType "1" (Consumer Services). The POS uses billTypes `"1"` (Consumer Services), `"4"` (Miscellaneous), and `"6"` (Clearance). The allocation search includes scoped tabs for various categories, with Institution search allowing discovery and allocation to linked accounts.
