    ...
    return (
      <div className="group flex items-start gap-2 sm:gap-3 py-2 sm:py-2.5 px-2 sm:px-3 rounded-lg hover:bg-[#F7F7F7] transition-all duration-200 cursor-default" data-testid={`field-${label.toLowerCase().replace(/\s+/g, '-')}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className={`text-[12px] sm:text-[13px] font-medium leading-snug break-words ${mono ? 'font-mono' : ''} ${accent ? 'text-[var(--pos-accent)]' : 'text-slate-800'} ${display === '-' ? 'text-slate-300' : ''}`}>{display}</div>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="p-4 sm:p-5 space-y-4" data-testid="account-info-panel">
      {error && !coreLoaded ? <ErrorState message={error} onRetry={load} /> : (
        <>
          <div className="bg-gradient-to-br from-[var(--pos-accent)] via-[var(--pos-accent-dark)] to-[var(--pos-accent-dark)] rounded-2xl shadow-lg overflow-hidden" data-testid="account-hero">
              ...
              <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2">
                    <span className="inline-flex items-center gap-1.5 text-[var(--pos-accent-light)] text-[13px]">
                      <CreditCard className="w-3.5 h-3.5" />
                      <span className="font-mono font-semibold text-white" data-testid="text-account-number">{accountNumber}</span>
                    </span>
                    {accountType !== '-' && (
                      <span className="inline-flex items-center gap-1.5 text-[var(--pos-accent-light)] text-[12px]">
                ...
                <div className="flex flex-col items-end gap-1 shrink-0">
                  {depositDisplay !== '-' && depositDisplay !== 'R 0.00' && (
                    <div className="bg-white/15 backdrop-blur-sm rounded-lg px-3 py-2 text-right">
                      <div className="text-[10px] uppercase tracking-wider text-[var(--pos-accent-light)] font-semibold">Deposit</div>
            ...
            <div className="bg-black/10 px-5 sm:px-6 py-3 flex flex-wrap gap-x-6 gap-y-2 text-[12px] text-[var(--pos-accent-light)]">
              {contactNo !== '-' && (
                    ...
                    {relatedAccounts.map((acc, idx) => {
                      ...
                      return (
                        <tr key={`${aid}-${idx}`} className="hover:bg-[#F7F7F7] transition-colors">
                          ...
                          <td className="px-4 py-2.5">
                            {onNavigateToAccount && (
                              <Button variant="ghost" size="sm" className="h-6 text-[10px] text-[var(--pos-accent)] hover:text-[var(--pos-accent-dark)] hover:bg-[var(--pos-accent-tint)] px-2" onClick={() => onNavigateToAccount(acc)} data-testid={`button-view-account-${aid}`}>
      ...
      {receiptPreview && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setReceiptPreview(null)} data-testid="receipt-preview-overlay">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
            <div className="px-5 py-3 bg-gradient-to-r from-[var(--pos-accent)] to-[var(--pos-accent-dark)] rounded-t-xl flex items-center justify-between">
      ...
      <div className="bg-white rounded-xl border border-[#D6D6D6] shadow-sm overflow-hidden">
        ...
        <div className="sm:hidden p-2 space-y-2">
          {balanceData.length === 0 ? (
            ...
            <>
              ...
              <div className="border-2 border-[#D6D6D6] rounded-lg p-3 bg-[var(--pos-accent-tint)]/50">
                <div className="flex items-center justify-between">
        ...
        <div className="hidden sm:block overflow-x-auto">
          <table className="w-full text-sm" data-testid="table-balance-debt">
            ...
            {balanceData.length > 0 && (
              <tfoot>
                <tr className="border-t-2 border-[#D6D6D6] bg-[var(--pos-accent-tint)]/50">
            ...
            <div key={i} className="border border-[#D6D6D6] rounded-lg p-2.5 space-y-1.5" data-testid={`card-payment-${i}`}>
              ...
              <div className="flex justify-end pt-1 border-t border-[#E5E5E5]">
                <button
                  ...
                  className="text-[var(--pos-accent)] hover:text-[var(--pos-accent-dark)] text-[10px] font-medium disabled:opacity-50 flex items-center gap-1"
                  ...
                  <tr key={i} className="border-b border-[#F7F7F7] hover:bg-green-50/50" data-testid={`row-payment-${i}`}>
                    ...
                    <td className="py-1.5 px-3 text-center">
                      <button
                        onClick={() => handlePrintReceipt(p)}
                        disabled={printingId === String(p.receiptId || p.receipt_ID || p.id)}
                        className="text-[var(--pos-accent)] hover:text-[var(--pos-accent-dark)] hover:underline text-xs font-medium disabled:opacity-50 flex items-center gap-1 mx-auto"
                        data-testid={`btn-print-receipt-${i}`}
                      >
                        {printingId === String(p.receiptId || p.receipt_ID || p.id) ? (
                          <><RefreshCw className="w-3 h-3 animate-spin" /> Loading...</>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {onSelectAccount && (
                        <button
                          ...
                          className="mt-2 px-3 py-1.5 text-[11px] font-medium bg-[var(--pos-accent-tint)] text-[var(--pos-accent)] border border-[#D6D6D6] rounded-md hover:bg-[var(--pos-accent-tint-strong)] transition-colors w-full sm:w-auto"
