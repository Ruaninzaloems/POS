...
export function SectionHeader({ title }: { title: string }) {
  return (
    <div className="flex items-center gap-3 py-2 sm:py-2.5 mt-1">
      ...
      <span className="text-[10px] sm:text-[11px] font-bold text-blue-700 uppercase tracking-[0.15em] whitespace-nowrap px-2">{title}</span>
    ...
    <div data-testid={`${tid}-container`}>
      <div className="sm:hidden space-y-2 p-2">
        {paged.length === 0 ? (
          ...
          <div
            key={i}
            className={`bg-white border border-slate-200 rounded-lg p-3 space-y-1.5 ${onRowClick ? 'cursor-pointer active:bg-blue-50' : ''}`}
    ...
    <>
      ...
      <div className="hidden sm:block overflow-x-auto">
        <table className="w-full text-xs" data-testid={testId}>
          ...
          <tbody>
            {data.map((row, i) => (
              <tr key={i} className="border-b border-slate-100 hover:bg-blue-50/30 transition-colors" data-testid={`${testId}-row-${i}`}>
                {columns.map(col => {
                  const content = col.render ? col.render(row) : (row[col.key] ?? '-');
                  return (
