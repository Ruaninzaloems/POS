...
import { ReceiptingConsumerPayments } from '@/components/pos/receipting-consumer-payments';
          ...
          return (
              <div className="flex-1 p-6 overflow-y-auto bg-gray-100/50">
                  <div className="max-w-[1200px] mx-auto space-y-6">
                      <ReceiptingConsumerPayments item={item} />
                  </div>
              </div>
          );
      }
...
function TransactionItemCard({ item }: { item: TransactionItem }) {
    ...
    if (item.type === 'CONSUMER_SERVICES') {
        return <ReceiptingConsumerPayments item={item} />;
    }
